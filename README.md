<div align="center"> 
  
# 📊 SPSS Statistics Course

### A structured, hands-on journey through SPSS, from raw data to real statistical insight.

[![SPSS](https://img.shields.io/badge/IBM%20SPSS-Statistics-0F62FE?style=for-the-badge&logo=ibm&logoColor=white)](https://www.ibm.com/spss)
[![License](https://img.shields.io/badge/License-MIT-green?style=for-the-badge)](LICENSE)
[![Progress](https://img.shields.io/badge/Progress-08%2F30%20Days-orange?style=for-the-badge)](#-days)
[![Status](https://img.shields.io/badge/Status-In%20Progress-yellow?style=for-the-badge)](#-days)

[![GitHub stars](https://img.shields.io/github/stars/HereIsMuhammad/spss-statistics-course?style=for-the-badge&color=yellow)](https://github.com/HereIsMuhammad/spss-statistics-course/stargazers)
[![GitHub forks](https://img.shields.io/github/forks/HereIsMuhammad/spss-statistics-course?style=for-the-badge&color=blue)](https://github.com/HereIsMuhammad/spss-statistics-course/network/members)
[![GitHub watchers](https://img.shields.io/github/watchers/HereIsMuhammad/spss-statistics-course?style=for-the-badge&color=orange)](https://github.com/HereIsMuhammad/spss-statistics-course/watchers)
[![GitHub issues](https://img.shields.io/github/issues/HereIsMuhammad/spss-statistics-course?style=for-the-badge&color=red)](https://github.com/HereIsMuhammad/spss-statistics-course/issues)

[![Last Commit](https://img.shields.io/github/last-commit/HereIsMuhammad/spss-statistics-course?style=flat-square)](https://github.com/HereIsMuhammad/spss-statistics-course/commits/main)
[![Contributors](https://img.shields.io/github/contributors/HereIsMuhammad/spss-statistics-course?style=flat-square)](https://github.com/HereIsMuhammad/spss-statistics-course/graphs/contributors)

</div>

## 📖 Introduction

This repository documents a complete, practical journey through **IBM SPSS Statistics**, from entering your first dataset to running regression, ANOVA, and factor analysis like a researcher.

Each module includes:
- 📝 **Notes**: a clear explanation of the concept and when to use it
- 📁 **Dataset**: a sample `.csv` / `.sav` file to practice with
- 🧮 **Syntax**: the SPSS syntax (`.sps`) behind the point-and-click steps
- 📊 **Output**: SPSS output tables/charts as PDF or screenshots
- 🧠 **Interpretation**: what the numbers actually mean, in plain language

## 📌 Table of Contents

- [Getting Started with SPSS (Days 1 to 4)](#-getting-started-with-spss-days-1-to-4)
- [Descriptive Statistics (Days 5 to 9)](#-descriptive-statistics-days-5-to-9)
- [Data Preparation (Days 10 to 13)](#-data-preparation-days-10-to-13)
- [Normality & Reliability (Days 14 to 16)](#-normality--reliability-days-14-to-16)
- [Correlation & Regression (Days 17 to 21)](#-correlation--regression-days-17-to-21)
- [Comparing Groups (Days 22 to 26)](#-comparing-groups-days-22-to-26)
- [Categorical & Advanced Analysis (Days 27 to 29)](#-categorical--advanced-analysis-days-27-to-29)
- [Capstone Project (Day 30)](#-capstone-project-day-30)
- [Tech Stack](#-tech-stack)
- [How to Use This Repo](#-how-to-use-this-repo)
- [License](#-license)

## 🚀 Getting Started with SPSS (Days 1 to 4)

<details>
<summary>Click to expand Days 1 to 4</summary>

| Day | Topic |
|---|---|
| 1 | What is SPSS, installation, and interface overview (Data View vs Variable View) |
| 2 | Creating a dataset: variable names, types, labels, and value labels |
| 3 | Importing data from Excel/CSV into SPSS |
| 4 | Understanding levels of measurement (Nominal, Ordinal, Scale) |

</details>

## 📊 Descriptive Statistics (Days 5 to 9)

<details>
<summary>Click to expand Days 5 to 9</summary>

| Day | Topic |
|---|---|
| 5 | Frequencies & Frequency Tables |
| 6 | Measures of Central Tendency: Mean, Median, Mode |
| 7 | Measures of Dispersion: Range, Variance, Standard Deviation |
| 8 | Skewness & Kurtosis |
| 9 | 🛠️ Project: Descriptive Summary of a Survey Dataset |

</details>

## 🧹 Data Preparation (Days 10 to 13)

<details>
<summary>Click to expand Days 10 to 13</summary>

| Day | Topic |
|---|---|
| 10 | Handling Missing Values |
| 11 | Recoding Variables (Recode into Same/Different Variables) |
| 12 | Computing New Variables |
| 13 | Data Cleaning & Outlier Detection |

</details>

## ✅ Normality & Reliability (Days 14 to 16)

<details>
<summary>Click to expand Days 14 to 16</summary>

| Day | Topic |
|---|---|
| 14 | Normality Testing (Shapiro-Wilk, Kolmogorov-Smirnov) |
| 15 | Reliability Analysis (Cronbach's Alpha) |
| 16 | 🛠️ Project: Validating a Questionnaire |

</details>

## 📈 Correlation & Regression (Days 17 to 21)

<details>
<summary>Click to expand Days 17 to 21</summary>

| Day | Topic |
|---|---|
| 17 | Pearson Correlation |
| 18 | Spearman Correlation (Non-parametric) |
| 19 | Simple Linear Regression |
| 20 | Multiple Linear Regression |
| 21 | 🛠️ Project: Predicting Exam Scores from Study Hours |

</details>

## ⚖️ Comparing Groups (Days 22 to 26)

<details>
<summary>Click to expand Days 22 to 26</summary>

| Day | Topic |
|---|---|
| 22 | Independent Samples T-Test |
| 23 | Paired Samples T-Test |
| 24 | One-Way ANOVA |
| 25 | Post-Hoc Tests (Tukey HSD) |
| 26 | 🛠️ Project: Comparing Test Scores Across Groups |

</details>

## 🧩 Categorical & Advanced Analysis (Days 27 to 29)

<details>
<summary>Click to expand Days 27 to 29</summary>

| Day | Topic |
|---|---|
| 27 | Chi-Square Test of Independence |
| 28 | Factor Analysis (Data Reduction) |
| 29 | Logistic Regression (Binary Outcomes) |

</details>

## 🏆 Capstone Project (Day 30)

<details>
<summary>Click to expand Day 30: End-to-End Survey Analysis</summary>

| Day | Topic |
|---|---|
| 30 | Capstone Project: Full survey dataset, cleaning, reliability, correlation, regression, and a written report |

</details>

## 🧰 Tech Stack

<div align="center">

![SPSS](https://img.shields.io/badge/IBM%20SPSS%20Statistics-0F62FE?style=for-the-badge&logo=ibm&logoColor=white)
![Excel](https://img.shields.io/badge/Microsoft%20Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white)
![CSV](https://img.shields.io/badge/CSV%20Data-4CAF50?style=for-the-badge)

</div>

## 📂 How to Use This Repo

Each day lives in its own folder, named `DayXX-Topic-Name/`, containing:
- `DayXX-Notes.md`: theory and explanation
- `dataset.csv` / `dataset.sav`: the practice data
- `syntax.sps`: SPSS syntax file
- `output.pdf`: SPSS output tables/charts
- `interpretation.md`: plain-language explanation of results

Clone the repo and explore any day:

```bash
git clone https://github.com/HereIsMuhammad/spss-statistics-course.git
cd spss-statistics-course
cd Day21-Regression-Exam-Scores
```

## 📜 License

This project is licensed under the MIT License. See the [LICENSE](LICENSE) file for details.

<div align="center">

### ⭐ If this repo helped you, consider giving it a star!

</div>
